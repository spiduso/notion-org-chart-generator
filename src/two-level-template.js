exports.createContentFromTwoLevelTemplate = (data, personName, relationName, personDescription) => {
    // name -> index v data
    const indexDict = {};
    // leader -> [name]
    const relationDict = {};
    ...
    for (const noLeadIndex of noLeaderArr) {
        ...
        content += getContentForPerson(leader, data, indexDict, relationDict, personName, 0, personDescription);
    }
...
function getContentForPerson(person, data, indexDict, relationDict, personName, depth, personDescription) {
    let level = 'level';
    if(depth > 0) {
        level = 'sub-level';
    }
    ...
    if (person[personName] in relationDict) {
        ...
        for (const employeeName of relationDict[person[personName]]) {
            ...
            content += getContentForPerson(employee, data, indexDict, relationDict, personName, depth + 1, personDescription);
        }
        ...
        for(const nameToResolve of firstToGet) {
            if(nameNotFound) {
                ...
                for (const name of relationDict[nameToResolve]) {
                    ...
                    content += getContentForPerson(person, data, indexDict, relationDict, personName, 1, personDescription);
                }
                content += '</ul>';
                content += '</li>';
            } else {
