exports.createContentFromTemplate = (data, personName, relationName) => {
    // name -> index v data
    const indexDict = {};
    // leader -> [name]
    const relationDict = {};
    ...
    let content = '<div style="display: inline-block" class="content"><ul>';
    for (const noLeadIndex of noLeaderArr) {
        const leader = data[noLeadIndex];
        content += getContentForPerson(leader, data, indexDict, relationDict, personName);
    }

    content += resolveNotFoundLeaders(data, indexDict, relationDict, personName);
...
function getContentForPerson(person, data, indexDict, relationDict, personName) {
    ...
    if (person[personName] in relationDict) {
        ...
        for (const employeeName of relationDict[person[personName]]) {
            ...
            content += getContentForPerson(employee, data, indexDict, relationDict, personName);
        }
